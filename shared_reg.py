...
def l2_regularization(lamda, model1, model2):

    iter = model1.parameters()
    l2_reg = None
    for W1 in model2.parameters():
        W2 = iter.next()
        if l2_reg is None:
            l2_reg = torch.norm(W1-W2, 2).pow(2)
        else:
            l2_reg = l2_reg + torch.norm(W1-W2, 2).pow(2)

    return l2_reg*lamda/2
    ...
    for idx in range(data.HP_iteration):
        ...
        for batch_id in range(total_batch):
            ...
            if batch_id < ner_total_batch:
                ...
                for i in range(opt.hidden_num):
                    ner_hidden = ner_hiddenlist[i].forward(ner_hidden, batch_wordlen)

                loss, tag_seq = seq_model.neg_log_likelihood_loss(ner_hidden, batch_label, mask)

                for i in range(opt.hidden_num):
                    if i > 0:
                        loss = loss + l2_regularization(opt.lamda, ner_hiddenlist[i], re_hiddenlist[i])
            ...
            if batch_id < re_total_batch:
                ...
                for i in range(opt.hidden_num):
                    re_hidden = re_hiddenlist[i].forward(re_hidden, batch_wordlen)
...
def ner_evaluate(data, wordrep, hiddenlist, model, name):
    ...
    for batch_id in range(total_batch):
        ...
        with torch.no_grad():
            ...
            ner_hidden = ner_word_rep
            for i in range(opt.hidden_num):
                ner_hidden = hiddenlist[i].forward(ner_hidden, batch_wordlen)
