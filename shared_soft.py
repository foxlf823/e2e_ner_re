...
class HiddenLayer(nn.Module):
    ...
    def forward(self, word_represent, word_seq_lengths, use_attn):
            ...
            if batch_id < min_batch:
                ...
                for i in range(opt.hidden_num):
                    if alpha > 0.99:
                        ...
                        ner_lstm_out, ner_att_out = ner_hiddenlist[i].forward(ner_hidden, ner_batch_wordlen, use_attn)
                        re_lstm_out, re_att_out = re_hiddenlist[i].forward(re_hidden, re_batch_wordlen, use_attn)
                        ...
                        ner_hidden = alpha*ner_lstm_out+(1-alpha)*re_att_out.unsqueeze(1)
                        re_hidden = alpha*re_lstm_out+(1-alpha)*ner_att_out.unsqueeze(1)
                ...
                if batch_id < ner_total_batch:
                    ...
                    ner_hidden = ner_word_rep
                    for i in range(opt.hidden_num):
                        ner_lstm_out, ner_att_out = ner_hiddenlist[i].forward(ner_hidden, batch_wordlen, False)
                        ner_hidden = ner_lstm_out
                ...
                if batch_id < re_total_batch:
                    ...
                    re_hidden = re_word_rep
                    for i in range(opt.hidden_num):
                        re_lstm_out, re_att_out = re_hiddenlist[i].forward(re_hidden, batch_wordlen, False)
                        re_hidden = re_lstm_out
...
def ner_evaluate(data, wordrep, hiddenlist, model, name):
    ...
    for batch_id in range(total_batch):
        ...
        with torch.no_grad():
            ...
            ner_hidden = ner_word_rep
            for i in range(opt.hidden_num):
                ner_lstm_out, ner_att_out = hiddenlist[i].forward(ner_hidden, batch_wordlen, False)
                ner_hidden = ner_lstm_out
